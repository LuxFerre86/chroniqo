...
public class MonthService {
    ...
    public List<DaySummaryDTO> getMonth(int year, int month) {
        ...
        List<TimeEntry> entries =
                timeEntryRepository.findByUserAndDateBetween(user, monthStart, monthEnd);

        List<Absence> absences =
                absenceService.getAbsences(user, monthStart, monthEnd);

        List<DaySummaryDTO> result = new ArrayList<>();

        int dailyTargetMinutes = (user.getWeeklyTargetHours() * 60) / 5;

        for (int i = 0; i < monthEnd.getDayOfMonth(); i++) {
